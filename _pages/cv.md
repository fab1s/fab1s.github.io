---
title: "Curriculum vitae"
permalink: /cv/
layout: single
author_profile: true
---



[Download my CV (PDF)](/files/fabio-schreiber-cv.pdf){: .btn .btn--primary}

## Experience

### Research Engineer · Lund University
<p class="role-date">Methods Centre, Sweden · May 2026–present</p>

Leading development of a research data platform for the Faculty of Social Sciences, enabling discovery and exploration of large datasets through agentic engineering. Leading a quantitative research project analysing infrastructure investment patterns across Brazil at national scale, applying computational and ecological methods to large geospatial datasets.

### Master Thesis Student · RISE
<p class="role-date">Research Institutes of Sweden · February–May 2025</p>

Applied GIS to assess equity in access to social infrastructure in Skåne County, using road network-based accessibility metrics and geographically weighted regression.

### Research Intern · RISE
<p class="role-date">Research Institutes of Sweden · August–October 2024</p>

Analysed and visualised spatial data for the City-Core Index project using GIS in R. Processed datasets, integrated APIs, and supported data-driven urban research.

### Project Assistant · Lund University
<p class="role-date">Sociology Department, Sweden · April–December 2024</p>

Used R to wrangle, analyse, and visualise ClinicalTrials.gov data, producing a dataset supporting a published paper and a submitted manuscript pending review.

## Education

### M.Sc. Social Scientific Data Analysis
<p class="role-date">Lund University, Sweden · September 2023–June 2025</p>

Interdisciplinary training in quantitative, theory-based data analysis, statistical methods, data handling in R, and research design.

**Thesis:** *Social Infrastructure and Structural (Dis)advantage: A GIS-Based Equity Assessment*

### B.A. Sociology · Minor in Economics
<p class="role-date">LMU Munich, Germany · 2019–2023</p>

Training in sociology and economics, with a foundation in social theory and computational tools for empirical research.

**Thesis:** *Assessing Public Perception of Sustainable Travel: A Reddit Analysis*

## Methods & tools

- **Programming:** R (advanced), SQL (intermediate), Python (beginner); agentic engineering via VS Code.
- **Data analysis:** tidyverse, ggplot2, dplyr; linear, logistic, and mixed regression; kNN, Naive Bayes, LDA, and text/sentiment analysis.
- **Geospatial & data collection:** sf, terra, GIS, web scraping with rvest, HTML, CSS, and XPath.
- **Reporting:** R Markdown, Quarto; Microsoft Excel, PowerPoint, and Word (intermediate).

## Languages

German and Portuguese (native) · English (fluent) · Mandarin (pre-intermediate, HSK3)

## Certifications

- DataCamp Data Scientist Associate Certificate in R
- DataCamp SQL Associate Certificate

## Contact

**Work:** [fabio.schreiber@keg.lu.se](mailto:fabio.schreiber@keg.lu.se)  
**Private:** [fabio1s.schreiber@gmail.com](mailto:fabio1s.schreiber@gmail.com)  
[Google Scholar](https://scholar.google.com/citations?user=e9tejNQAAAAJ) · [LinkedIn](https://www.linkedin.com/in/fabio-schreiber-1b79972a7/) · [GitHub](https://github.com/fab1s)

